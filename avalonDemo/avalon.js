...
new function() {
    ...
    function checkLoadingComplete(){
    	if(jsLoading==0){
    		window.Tips&&(Tips.hideLoading());
    	}
    }
    function loadJS(url, id, callback) {
    	if(jsLoading++==0){
    		window.Tips&&(Tips.showLoading());
    	}
        //通过script节点加载目标模块
        ...
        node.src = url+"?v="+version; //插入到head的第一个节点前，防止IE6下head标签没闭合前使用appendChild抛错
