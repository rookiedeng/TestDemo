 ...
 *                    全局变量及方法                                  *
 **********************************************************************/
var expose = new Date - 0 //得到时间戳
//http://stackoverflow.com/questions/7290086/javascript-use-strict-and-nicks-find-global-function
var DOC = window.document
var head = DOC.getElementsByTagName("head")[0] //HEAD元素
//在head标签中的第一个子元素加载avalon标签
...
//<avalon ms-skip="1">X<style id="avalonStyle">.avalonHide{ display: none!important }</style></avalon>
...
///^\[object (?:Window|DOMWindow|global)\]$/.test(Object.prototype.toString.call(window)) //true
...
/*
 ...
 事件触发器就是用来触发某个元素下的某个事件，IE下fireEvent方法，
 高级浏览器（chrome,firefox等）有dispatchEvent方法。
一般我们在元素上绑定事件后，是靠用户在这些元素上的鼠标行为来捕获或者触发事件的，
或者自带的浏览器行为事件，比如click，mouseover，load等等，
有些时候我们需要自定义事件或者在特定的情况下需要触发这些事件。
...
在这段代码中，每个新的<p />元素都被添加到文档碎片中，然后这个碎片被作为参数传递给appendChild()。
这里对appendChild()的调用实际上并不是把文档碎片本省追加到body元素中，而是仅仅追加碎片中的子节点，然后可以看到明显的性能提升，
document.body.appenChild()一次替代十次，这意味着只需要进行一个内容渲染刷新。*/

var cinerator = DOC.createElement("div")
var class2type = {} //保存JavaScript中的所有数据类型
...
/*
 例子如下：
oneObject(["", "text/javascript", "text/ecmascript", "application/ecmascript", "application/javascript"])
Object {"": 1, text/javascript: 1, text/ecmascript: 1, application/ecmascript: 1, application/javascript: 1}
*/
...
function createCache(maxLength) {
    var keys = []
    function cache(key, value) {
        if (keys.push(key) > maxLength) { //当缓存的个数大于最大的时候，移除最后一个
...
//判断IE的版本
function IE() {
	//window.VBArray：判断是否为IE   在IE下就是本地函数 其他情况下为undefined
    if (window.VBArray) {
        var mode = document.documentMode  //判断IE的版本（在IE8+以上有效）
...
function kernel(settings) {
    for (var p in settings) {      //遍历出配置对象中的各个属性
...
new function() {
   ...
   //处理要加载资源的链接
    function loadResources(url, parentUrl) {
        ...
        return plugin(url, shim)//找到加载资源的函数
    }
 ...
 * 如果页面上有大图片等资源响应缓慢, 会导致window.onload事件迟迟无法触发.
 * 所以出现了DOM Ready事件. 此事件在DOM文档结构准备完毕后触发, 即在资源加载前触发. 
 * 另外我们需要在DOM准备完毕后, 再修改DOM结构, 比如添加DOM元素等. 
 * 否则有可能出现“Internet Explorer无法打开站点”的问题. *
 **********************************************************************/

var readyList = []
//当dom树构建完毕后就执行的函数fireReady
function fireReady() {
...
//暴露avalon 和 $ 标示，即使在amd和commonjs 浏览器模拟器中
    if (noGlobal === void 0) {
        window.avalon = avalon
    }
    return avalon
    //void 0 后面不管跟着什么 都会返回undefined
    //扩展知识：http://www.cnblogs.com/fsjohnhuang/p/4146506.html
